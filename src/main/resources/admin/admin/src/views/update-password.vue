    ...
    onUpdateHandler() {
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          ...
          if (this.$storage.get("sessionTable") == 'huodongzuzhizhe') {
			if(this.ruleForm.newpassword && (this.ruleForm.newpassword.length<2)){
				this.$message.error(`新密码长度不能小于2`);
				return
			}
			if(this.ruleForm.newpassword && (this.ruleForm.newpassword.length>10)){
				this.$message.error(`新密码长度不能大于10`);
				return
			}
          }
