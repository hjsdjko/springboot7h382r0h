<template>
	<div class="main-content" :style='{"padding":"20px 30px 30px","margin":"0 auto","color":"#666","borderRadius":"0 0 10px 10px","background":"#fff","width":"calc(100% - 60px)","fontSize":"16px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px 10px 0","margin":"0px","fontSize":"inherit","overflow":"hidden","background":"none"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"width":"100%","margin":"10px 0 10PX","color":"rgba(19,161,230,1)","flexWrap":"wrap","display":"flex"}'>
					<el-button class="add" v-if="isAuth('huodongshenqing','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia13" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('huodongshenqing','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu7" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('huodongshenqing','审核')" :disabled="dataListSelections.length?false:true" type="success" @click="shBatchDialog()">
			...
			<div :style='{"border":"1px solid rgba(19,161,230,.2)","width":"100%","padding":"0","borderRadius":"0px","background":"#fff"}'>
				<el-table class="tables"
					...
					:border='false'
					v-if="isAuth('huodongshenqing','查看')"
					:data="dataList"
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="xueshengxingbie"
						label="学生性别">
						<template slot-scope="scope">
							{{scope.row.xueshengxingbie}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="nianling"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('huodongshenqing','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan14" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								更多
							</el-button>
							<el-button class="btn8" v-if="isAuth('huodongshenqing','取消')" @click="quxiaoshenqingCrossAddOrUpdateHandler(scope.row,'cross','是','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								取消
							</el-button>
							<el-button class="btn8" v-if="isAuth('huodongshenqing','记录')" @click="huodongrizhiCrossAddOrUpdateHandler(scope.row,'cross','是','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								记录
							</el-button>
							<el-button class="edit" v-if=" isAuth('huodongshenqing','修改')  && scope.row.sfsh=='待审核' " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('huodongshenqing','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<quxiaoshenqing-cross-add-or-update v-if="quxiaoshenqingCrossAddOrUpdateFlag" :parent="this" ref="quxiaoshenqingCrossaddOrUpdate"></quxiaoshenqing-cross-add-or-update>
		<huodongrizhi-cross-add-or-update v-if="huodongrizhiCrossAddOrUpdateFlag" :parent="this" ref="huodongrizhiCrossaddOrUpdate"></huodongrizhi-cross-add-or-update>
...
import quxiaoshenqingCrossAddOrUpdate from "../quxiaoshenqing/add-or-update";
import huodongrizhiCrossAddOrUpdate from "../huodongrizhi/add-or-update";
	export default {
		data() {
			return {
				searchForm: {
				...
				quxiaoshenqingCrossAddOrUpdateFlag: false,
				huodongrizhiCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			quxiaoshenqingCrossAddOrUpdate,
			huodongrizhiCrossAddOrUpdate,
		},
		methods: {
			...
			quxiaoshenqingCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				this.quxiaoshenqingCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','huodongshenqing');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.quxiaoshenqingCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.quxiaoshenqingCrossaddOrUpdate.init(row.id,type);
				});
			},
			huodongrizhiCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
				if(crossOptAudit=='是'&&row.sfsh!='是') {
					this.$message({
						message: "请审核通过后再操作",
						type: "success",
				...
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.huodongrizhiCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','huodongshenqing');
				this.$storage.set('statusColumnName',statusColumnName);
				this.$storage.set('statusColumnValue',statusColumnValue);
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.huodongrizhiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.huodongrizhiCrossaddOrUpdate.init(row.id,type);
				});
			},
    ...
    shBatchHandler(){
      this.$confirm(`是否${this.batchIds.length>1?'一键审核':'审核'}选中数据?`, "提示", {
        ...
        this.$http({
          url: "huodongshenqing/shBatch?sfsh="+this.shBatchForm.sfsh+"&shhf="+this.shBatchForm.shhf,
          method: "post",
          data: this.batchIds
        }).then(({ data }) => {
          if (data && data.code === 0) {
    ...
    download(file){
      ...
      axios.get(location.href.split(this.$base.name)[0] + this.$base.name + '/file/download?fileName=' + arr, {
      	headers: {
	...
	huodongshenqingstatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'huodongshenqing/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
