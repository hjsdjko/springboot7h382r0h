<template>
	<div class="main-content" :style='{"padding":"20px 30px 30px","margin":"0 auto","color":"#666","borderRadius":"0 0 10px 10px","background":"#fff","width":"calc(100% - 60px)","fontSize":"16px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px 10px 0","margin":"0px","fontSize":"inherit","overflow":"hidden","background":"none"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"width":"100%","margin":"10px 0 10PX","color":"rgba(19,161,230,1)","flexWrap":"wrap","display":"flex"}'>
					<el-button class="add" v-if="isAuth('dushuhuihuodong','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia13" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('dushuhuihuodong','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"border":"1px solid rgba(19,161,230,.2)","width":"100%","padding":"0","borderRadius":"0px","background":"#fff"}'>
				<el-table class="tables"
					...
					:border='false'
					v-if="isAuth('dushuhuihuodong','查看')"
					:data="dataList"
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('dushuhuihuodong','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan14" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								更多
							</el-button>
							<el-button class="btn8" v-if="isAuth('dushuhuihuodong','申请')" @click="huodongshenqingCrossAddOrUpdateHandler(scope.row,'cross','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								申请
							</el-button>
							<el-button class="edit" v-if=" isAuth('dushuhuihuodong','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xiugai10" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								改动
							</el-button>

							<el-button class="view" v-if="isAuth('dushuhuihuodong','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
								<span class="icon iconfont icon-chakan14" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								查看评论
							</el-button>
							...
							<el-button class="del" v-if="isAuth('dushuhuihuodong','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<huodongshenqing-cross-add-or-update v-if="huodongshenqingCrossAddOrUpdateFlag" :parent="this" ref="huodongshenqingCrossaddOrUpdate"></huodongshenqing-cross-add-or-update>
...
import huodongshenqingCrossAddOrUpdate from "../huodongshenqing/add-or-update";
	export default {
		data() {
			return {
				searchForm: {
				...
				huodongshenqingCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			huodongshenqingCrossAddOrUpdate,
		},
		methods: {
			...
			huodongshenqingCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.huodongshenqingCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','dushuhuihuodong');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.huodongshenqingCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.huodongshenqingCrossaddOrUpdate.init(row.id,type);
				});
			},
	...
	this.$router.push({path:'/discussdushuhuihuodong',query:{refid:id}});
    },
    // 下载
    download(file){
	...
	dushuhuihuodongstatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'dushuhuihuodong/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
