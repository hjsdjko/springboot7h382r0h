	...
	<div class="addEdit-block" :style='{"padding":"20px 30px 30px","margin":"0 auto","color":"#666","borderRadius":"0 0 10px 10px","background":"#fff","width":"calc(100% - 60px)","fontSize":"16px"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"color":"inherit","margin":"0 0 20px 0","fontSize":"inherit"}' class="upload" v-if="type!='info' && !ro.huodongtupian" label="活动图片" prop="huodongtupian">
					<file-upload
						tip="点击上传活动图片"
						action="file/upload"
						:limit="3"
				...
				<el-form-item :style='{"color":"inherit","margin":"0 0 20px 0","fontSize":"inherit"}' class="upload" v-else-if="ruleForm.huodongtupian" label="活动图片" prop="huodongtupian">
				...
				<el-form-item :style='{"color":"inherit","margin":"0 0 20px 0","fontSize":"inherit"}' class="date" v-if="type!='info'" label="记录时间" prop="jilushijian">
					<el-date-picker
				...
				<el-form-item :style='{"color":"inherit","margin":"0 0 20px 0","fontSize":"inherit"}' class="input" v-else-if="ruleForm.jilushijian" label="记录时间" prop="jilushijian">
				...
				<el-form-item :style='{"color":"inherit","margin":"0 0 20px 0","fontSize":"inherit"}' v-if="type!='info'"  label="活动经历" prop="huodongjingli">
					<editor 
						style="min-width: 200px; max-width: 600px;"
						v-model="ruleForm.huodongjingli" 
						class="editor" 
						action="file/upload">
					</editor>
				</el-form-item>
				<el-form-item :style='{"color":"inherit","margin":"0 0 20px 0","fontSize":"inherit"}' v-else-if="ruleForm.huodongjingli" label="活动经历" prop="huodongjingli">
                    <span :style='{"fontSize":"16px","lineHeight":"40px","color":"inherit","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.huodongjingli"></span>
                </el-form-item>
...
export default {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='huodongjingli'){
							this.ruleForm.huodongjingli = obj[o];
							this.ro.huodongjingli = true;
							continue;
						}
						if(o=='huodongdidian'){
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.huodongjingli = this.ruleForm.huodongjingli.replace(reg,'../../../springboot7h382r0h/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `huodongrizhi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.huodongrizhiCrossAddOrUpdateFlag = false;
						 this.parent.search();
